...
def get_min_axis(image: np.ndarray) -> int:
...
def process_timelapse(image: np.ndarray) -> np.ndarray:
    """
...
def process_zstack(image: np.ndarray) -> np.ndarray:
    """
...
def process_standard(image: np.ndarray) -> np.ndarray:
    """ Returns squeezed image. """
    return image.squeeze()


def save_image(dir_base: dir, basename: str, image: np.ndarray) -> None:
...
@click.option(
    "--timelapse", prompt="Timelapse", is_flag=True, help="If images are timelapses."
)
@click.option("--zstack", prompt="ZStack", is_flag=True, help="If images are zstacks.")
def main(
    dir_base: dir,
    allslices: bool = False,
    ...
    log.info(
        f"Started with dir_base '{dir_base}', timelapse '{timelapse}', zstack '{zstack}'."
    )
