...
def model_seg(input_shape, n_labels=2):
    ...
    conv10 = conv2d_bn(mresblock9, n_labels, 1, 1, activation='sigmoid')
...
def conv2d_bn(x, filters, num_row, num_col, padding='same', strides=(1, 1), activation=None):
    '''
...
def trans_conv2d_bn(x, filters, kernel_size, padding='same', strides=(2, 2)):
    '''
    ...
    x = keras.layers.Conv2DTranspose(filters, (kernel_size, kernel_size), strides=strides, padding=padding)(x)
    x = keras.layers.BatchNormalization(axis=3, scale=False)(x)
