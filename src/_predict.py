...
def predict_baseline(
    ...
    if not isinstance(image, np.ndarray):
        raise TypeError(f"image must be of type np.ndarray but is {type(image)}.")
    if not isinstance(model, tf.keras.models.Model):
        raise TypeError(f"model must be of type np.ndarray but is {type(model)}.")
    ...
    def __next_power(x: float, k: int = 2):
...
def add_instances(pred_mask: np.ndarray) -> np.ndarray:
    ...
    Args:
        - pred_mask: Prediction mask to for instances should be added.
    Returns:
        - pred: 2D mask containing unique values for every instance.
    """
